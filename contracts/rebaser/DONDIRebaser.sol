...
contract DONDIRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 dondisSold, uint256 dondisFromReserves, uint256 dondisToReserves);
    ...
    /// @notice DONDI token address
    address public dondiAddress;
    ...
    /// @notice pair for reserveToken <> DONDI
    ...
    /// @notice Whether or not this token is first in uniswap DONDI<>Reserve pair
    bool public isToken0;

    constructor(
        address dondiAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(dondiAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == dondiAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap DONDI<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        DONDITokenInterface dondi = DONDITokenInterface(dondiAddress);

        if (positive) {
            require(dondi.dondisScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < dondi.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = dondi.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = dondi.rebase(epoch, indexDelta, positive);
        assert(dondi.dondisScalingFactor() <= dondi.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            dondi.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.dondisToUni) {
                // if the amount from reserves > dondisToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                dondi.mint(uniswap_pair, uniVars.dondisToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            dondi.mint(uniswap_pair, uniVars.dondisToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            dondi.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.dondisToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.dondisToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess dondi in the reserve
        uint256 excess = dondi.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          dondisToUni: tokens_to_max_slippage, // how many dondis uniswap needs
          amountFromReserves: excess, // how much of dondisToUni comes from reserves
          mintToReserves: 0 // how much dondis protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.dondisToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire dondi reserves, and minting part of total
